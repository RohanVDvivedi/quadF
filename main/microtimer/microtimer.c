    ...
    if(intr_status & BIT(0))
    {
        ...
        for(uint8_t i = 0; i < MAX_TIMER_EVENTS; i++)
        {
            if(timer_events_informations[i].enabled)
            {
                ...
                now_ticks_count = (((uint64_t) TIMERG0.hw_timer[0].cnt_high) << 32) | TIMERG0.hw_timer[0].cnt_low;
                if(timer_events_informations[i].next_occurence < minimum_next_occurence_value || minimum_next_occurence_value <= now_ticks_count)
                {
...
{
    printf("every : %llu, last : %llu, next : %llu\n",
    timer_events_informations[timer_event_no].every_x_ticks,
    timer_events_informations[timer_event_no].last_occurence,
    timer_events_informations[timer_event_no].next_occurence);
}
